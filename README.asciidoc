...
[[tasks]]
=== Tasks (a task)
Each <<project,projects>> is made up of one or more tasks. Each task is atomic, with inputs and outputs. Gradle executes tasks to perform its work. Task examples include; compiling classes, creating a JAR/APK, generating Javadoc, running static analysis (lint), deleting temporary files, or publishing to a repository, etc. When a task is asked to run we can see the outcome of the task. This will be one of EXECUTED, SKIPPED, FAILED, FROM-CACHE, UP-TO-DATE, NO-SOURCE or _blank_ (meaning executed). 
